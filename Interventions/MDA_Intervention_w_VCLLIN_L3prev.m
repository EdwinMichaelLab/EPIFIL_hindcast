function [MfAgePrevStore,CfaAgePrevStore,MfPrevStore,CfaPrevStore,MBRArray,L3Array] = ...
    ...
    PP,alpha2,gamma2,k0_c,kLin_c,k0_l3,kLin_l3,ageMthMax,da,bCulex,l3,gVec,pVec,...
...
MfAgePrevStore = zeros(ageMthMax,1,'single');
CfaAgePrevStore = zeros(ageMthMax,1,'single');
iYear = 0;
...
CfaPrevStore(1) = 100*mfAgeprevFun(sumC,negBinshapeFun(sumC,k0_c,kLin_c));

L3Array(1, iMbr+1)=100*mfAgeprevFun(sum(l3),negBinshapeFun(sumM,k0_l3,kLin_l3));%sum(l3);
iMbr=iMbr+1;
...
for iTime = 1:da:NumYears*12
    ...
    if (mod(iTime,MDAInterval(iTime)) == 1) || MDAInterval(iTime) == 1
        sinceTreat=1;
        if iTime == (APYear-1)*12+1 %iYear+1 == APYear
            MfAgePrevStore(:,1) = 100*mfAgeprevFun(m2,negBinshapeFun(m2,k0,kLin));
            CfaAgePrevStore(:,1) = 100*mfAgeprevFun(c2,negBinshapeFun(c2,k0_c,kLin_c));
        end
        iYear = iYear + 1;
...
%     iMbr = iMbr+1;
    ...
    CfaPrevStore(1,iTime+1) = 100*mfAgeprevFun(sumC,negBinshapeFun(sumC,k0_c,kLin_c));
    
    L3Array(1, iMbr+1) = 100*mfAgeprevFun(sum(l3),negBinshapeFun(sumM,k0_l3,kLin_l3));
    iMbr = iMbr+1;
